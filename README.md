...
- **Meta-Modell + Validator (K1–K3, D1–D4, Z1–Z4, A1–A3, C1–C3, H1–H4/F1–F4) + Change Operations** (`serial`/`parallel`/`conditional insert`, `add_data_element`, `connect_data`, `add_role`/`add_org_unit`/`add_agent`, `assign_service`, `assign_staff_rule`, `add_activity_template`, `register_connector`, `bind_external_data`, `insert_subprocess`, `link_follow_up`, `release`) + **BPMN-Import/Export** (`export_bpmn`/`import_bpmn`).
...
- **Web-Client** ([web/](web/)): schlanker **No-Build**-API-Client (reines HTML/CSS/JS, kein Bundler) als dünne GUI mit sechs Sichten — Modellieren (geführte +-Operationen, live validiert), Datensicht, Ressourcensicht (Organisation als **Baumstruktur** mit Abteilungen, Vorgesetzten und Umhängen-Dialog; Agenten samt Vertreter in eigener Tabelle), Ausführung (Worklist + Live-Prozesslandkarte), Meine Aufgaben (Bearbeiter-Aufgabenliste) und Monitoring. Jede Mutation läuft über den Validate-before-Commit-Pfad des Kerns; die GUI trägt **keine** Korrektheitslogik.
- **Persistenz**: austauschbarer Store für Schemata, Instanzen und das Event-Log — in-memory ohne Konfiguration, PostgreSQL via `DATABASE_URL` (SQLAlchemy 2.0, JSONB, Alembic-Migrationen `0001`/`0002`/`0003`, docker-compose); Instanzen und Audit-Verlauf sind durabel.
...
- web/         # Schlanker No-Build-API-Client (HTML/CSS/JS, sechs Sichten)
